...
app.get("/v1/codex/app-server/events/:threadId", async (request, reply) => {
  ...
  try { for await (const event of appServer.events(threadId)) reply.raw.write(`${JSON.stringify(event)}\n`); } finally { reply.raw.end(); }
});
