...
export function SummaryReport({ status, tasks, agents, messages, events }: Props) {
  ...
  return <div className="summary-report"><div className="section-heading"><h3>Summary Report So Far</h3><span>live workstream snapshot</span></div><p className="summary-narrative">The Workstream is <strong>{status.replaceAll("_", " ")}</strong>. {latest}</p><div className="summary-stats"><div><strong>{done}/{tasks.length}</strong><small>Tasks done</small></div><div><strong>{agents.filter((agent) => agent.status === "running").length}</strong><small>Agents running</small></div><div><strong>{messages.length}</strong><small>Messages</small></div><div><strong>{evidence}</strong><small>Evidence items</small></div></div><div className="summary-details"><div><strong>Current attention</strong><p>{attention}</p></div><div><strong>Agent coverage</strong><p>{agents.map((agent) => `${agent.role.toUpperCase()} · ${agent.status}`).join("  /  ")}</p></div></div></div>;
}
