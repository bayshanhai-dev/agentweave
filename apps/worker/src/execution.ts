...
export class AgentTaskExecutor {
  ...
  async execute(task: AgentTask, control = new ExecutionControl()): Promise<void> {
    ...
    try {
      ...
      const run = this.provider.run({ session, input: task.prompt, ...(task.model ? { model: task.model } : {}), ...(task.workspacePath ? { workspacePath: task.workspacePath } : {}), ...(task.correlationId ? { correlationId: task.correlationId } : {}), idempotencyKey: task.idempotencyKey ?? task.taskId });
      ...
      result = step.value;
      if (result) { control.assertRunnable(); record.status = "completed"; record.currentTurnId = result.turnId; record.lastCheckpoint = await this.provider.checkpoint(result.session); await this.sessions.save(record); await this.sink({ type: "task.completed", taskId: task.taskId, text: result.text }); }
