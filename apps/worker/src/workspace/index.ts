...
export async function persistWorkspaceEvidence(evidence: WorkspaceEvidence): Promise<string> { const sql = postgres(process.env.DATABASE_URL ?? "postgres://agentweave:agentweave@localhost:5432/agentweave"); const rows = await sql`insert into workspace_evidence (task_id, workspace_path, git_diff, kind, warnings, test_command, test_output, test_exit_code, created_at) values (${evidence.taskId}, ${evidence.workspacePath}, ${evidence.gitDiff}, ${evidence.kind ?? "workspace"}, ${JSON.stringify(evidence.warnings ?? [])}::jsonb, ${evidence.testCommand ?? null}, ${evidence.testOutput ?? null}, ${evidence.testExitCode ?? null}, ${evidence.createdAt}) returning id`; await sql.end(); return String(rows[0]?.id ?? `${evidence.taskId}:evidence`); }
