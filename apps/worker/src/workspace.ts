...
export async function persistWorkspaceEvidence(evidence: WorkspaceEvidence): Promise<void> { const sql = postgres(process.env.DATABASE_URL ?? "postgres://agentweave:agentweave@localhost:5432/agentweave"); await sql`insert into workspace_evidence (task_id, workspace_path, git_diff, test_command, test_output, test_exit_code, created_at) values (${evidence.taskId}, ${evidence.workspacePath}, ${evidence.gitDiff}, ${evidence.testCommand ?? null}, ${evidence.testOutput ?? null}, ${evidence.testExitCode ?? null}, ${evidence.createdAt})`; await sql.end(); }
